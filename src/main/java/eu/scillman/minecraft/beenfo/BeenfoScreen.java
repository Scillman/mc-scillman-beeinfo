...
public class BeenfoScreen extends Screen
{
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta)
    {
        ...
        {
            drawTexture(matrices, x, y, 0, 0, 176, 30); // header

            int minRows = getMinBeeRows();
            for (int i = 0; i < minRows; i++)
            {
                drawTexture(matrices, x, y+30+(i*30), 0, 30, 176, 30); // icon+name background

                if (i < beeNames.size())
                {
                    drawTexture(matrices, x+9, y+32+(i*30), 0, 166, 22, 22); // icon
                }
            }
            ...
            {
                drawTexture(matrices, x+7+(i*18), y+7, 8, 64, 18, 18); // bottle slots
            }
        }
